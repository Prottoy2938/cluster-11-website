...
test("should take a snapshot", () => {
  const { asFragment } = render(<SearchBar recipeName="chicken" />);
  expect(asFragment(<SearchBar recipeName="chicken" />)).toMatchSnapshot();
});
...
test("Shows search suggestions", async () => {
  const { getByTestId } = render(<SearchBar recipeName="chicken" />);
